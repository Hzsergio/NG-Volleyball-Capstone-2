...
const UserChallenges = () => {
  ...
  useEffect(() => {
    const fetchUserChallenges = async () => {
      try {
        if (userInfo) {
          const response = await axios.get(`http://localhost:8000/MatchTable/user_challenges/${userInfo.id}/`);
          ...
          const challengesWithSchedule = await Promise.all(
            response.data.map(async (challenge) => {
              ...
              const scheduleResponse = await axios.get(`http://localhost:8000/CourtSchedule/match-court/${challenge.id}/`);
      ...
      <div className="division-container">
        {userChallenges.map((challenge) => (
          <div key={challenge.id} className="team-box">
            ...
            <p>Status: {challenge.status === 's' ? 'Scheduled' : challenge.status === 'i' ? 'In Progress' : challenge.status}</p>
            {/* Display schedule details if available */}
            {challenge.courtSchedules && (
              <div>
                <p>Location: <br/> {challenge.courtSchedules.location}</p>
